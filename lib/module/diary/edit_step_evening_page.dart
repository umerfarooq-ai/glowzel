...
class EditStepsEveningPage extends StatefulWidget {
  final List<bool> initialToggles;
  final DateTime selectedDate;
  const EditStepsEveningPage({super.key, required this.initialToggles, required this.selectedDate});

  @override
  State<EditStepsEveningPage> createState() => _EditStepsEveningPageState();
}

class _EditStepsEveningPageState extends State<EditStepsEveningPage> {
  ...
  void loadToggles() async {
    final saved = await sessionRepository.getEveningToggleForDate(widget.selectedDate);
  ...
  Future<SkinAnalysisResponse?> fetchSkinHealthForDate(DateTime date) async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (result != null) {
          steps = (result.pmRoutine as List<dynamic>? ?? [])
  ...
  void saveRoutineToggles() async {
    ...
    await sessionRepository.setEveningToggleForDate(widget.selectedDate, stepsToggles);
  }
  ...
  Widget build(BuildContext context) {
    return FutureBuilder<SkinAnalysisResponse?>(
      ...
      builder: (context, snapshot) {
        ...
        if (steps.isEmpty) {
          final apiSteps = (skinHealthData.pmRoutine as List<dynamic>?)
              ?.map<Map<String, dynamic>>((step) => {
