...
class MorningRoutinePage extends StatefulWidget {
  ...
  const MorningRoutinePage({super.key, required this.selectedDate});
...
class _MorningRoutinePageState extends State<MorningRoutinePage> {
  List<Map<String, dynamic>> allSteps = []; // All steps including API + custom
  List<Map<String, dynamic>> visibleSteps = []; // Only steps based on toggles
  ...
  void loadSteps() async {
    final savedToggles =
        await sessionRepository.getMorningToggleForDate(widget.selectedDate);
      ...
      if (response.statusCode == 200) {
        ...
        if (analyses.isNotEmpty) {
          ...
          if (analysisForDate.isNotEmpty) {
            final stepsData =
                analysisForDate['amRoutine']?['steps'] as List<dynamic>? ?? [];
            apiSteps = stepsData
                .map((step) => {
  ...
  void saveToggles() async {
    await sessionRepository.setMorningToggleForDate(
        ...
        child: Column(
          children: [
            ...
            SizedBox(
              ...
              child: Stack(
                children: [
                  Image.asset('assets/images/png/morning.png',
                      width: double.infinity,
                      height: 250,
                      fit: BoxFit.cover,
                    ...
                    child: Center(
                      child: Column(
                        children: [
                          ...
                          Text(
                            'Refresh, protect, and prep skin for the day.',
                          ...
                          SvgPicture.asset('assets/images/svg/morning.svg'),
                        ],
                      ),
                    ),
                  ),
